...
$(document).ready(function () {
    ...
    d3.json("static/data/world-110m.json", function (error, world) {
        ...
        var country = svg.selectAll(".country")
            ...
            .enter().append("g")
            .attr("id", function(d){    //Set id of the shape as the country name
                return d.id;
            })
            .attr("class", "country");
        // Countries' shape
    ...
    function boundsPolygon(b) {
        return function (geometry) {
            ...
            if (geo_side_max < 1000 && geo_side_max >= 100) {
                digits = 1;
            } else if (geo_side_max < 100 && geo_side_max >= 10) {
                digits = 2;
            } else if (geo_side_max < 10 && geo_side_max >= 1) {
                digits = 3;
            } else if (geo_side_max < 1 && geo_side_max >= 0.1) {
                digits = 4;
            } else if (geo_side_max < 0.1 && geo_side_max >= 0.01) {
                digits = 5;
            } else if (geo_side_max < 0.01 && geo_side_max >= 0.001) {
                digits = 6;
            } else if (geo_side_max < 0.001 && geo_side_max >= 0.0001) {
                digits = 7;
            };

            //PRINT RESULTS (4 meaningful digits): 
            //console.log('Bound WNES: "' + WNES.id + '":{ ' + '"item":  { "name":"' + WNES.id + '", "lat_center":' + WNES.lat_center.toFixed(digits) + ',"lon_center":' + WNES.lon_center.toFixed(digits) + '},' + '"WNES":  { "W":' + WNES.W.toFixed(digits) + ',"N":' + WNES.N.toFixed(digits) + ',"E":' + WNES.E.toFixed(digits) + ',"S":' + WNES.S.toFixed(digits) + ',"geo_width":' + WNES.geo_width.toFixed(digits) + ',"geo_height":' + WNES.geo_height.toFixed(digits) + '},' + '"WNES+": { "W+":' + WNESplus.W.toFixed(digits) + ',"N+":' + WNESplus.N.toFixed(digits) + ',"E+":' + WNESplus.E.toFixed(digits) + ',"S+":' + WNESplus.S.toFixed(digits) + ',"geo_width+":' + WNESplus.geo_width.toFixed(digits) + ',"geo_height+":' + WNESplus.geo_height.toFixed(digits) + '}' + '},');
            // END WNES JSON #########################################################################################################
    ...
    function parallel(φ, λ0, λ1) {
        ...
        return d3.range(λ0, λ1 + .5 * step, step).map(function (λ) {
